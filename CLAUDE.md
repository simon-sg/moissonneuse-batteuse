...
- **Nœud RUDI card is the one exception to "go through `cli.py`"**: it calls `connectors/rudi_node.py`'s `statut_conteneur()`/`demarrer_conteneur()`/`arreter_conteneur()` directly (`GET /api/noeud`, `POST /api/noeud/<demarrer|arreter>`), because there's no terminal equivalent in `cli.py` to reuse — Podman container control has no CLI menu entry today. The container name is `CONTENEUR_RUDI` in `rudi_node.py` (currently `"rudinode"`) — update it there if the local node container is ever renamed. This card also links out to the node's manager UI (`{rudi_node.json url}/manager/`) and to the generated `data/catalogue.html` via a plain `file://` link — the catalogue is designed to work opened straight from disk (see `catalogue.py`'s progressive-loading design), so no static file serving was needed on the dashboard side.
