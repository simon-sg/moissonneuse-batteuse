...
- [Tests](#tests)
- [Monitoring & Superset (optionnel)](#monitoring--superset-optionnel)
...
6. **Monitoring (optionnel)** — alimentation d'une base PostGIS visualisée dans Superset (volumes moissonnés, historique, référentiels, géocodage).
...
| Composant | Rôle |
|---|---|
| `src/cli.py` | Entrée terminal — menu de 21 actions (moisson, pipeline, maintenance, données) |
...
| `src/monitor.py` | Alimentation de la base PostGIS de monitoring (optionnel) |
...
```bash
pip install psycopg2-binary   # monitoring PostGIS uniquement (src/monitor.py)
...
Pour le monitoring optionnel (PostGIS + Superset), Docker et Docker Compose sont nécessaires. Pour un nœud RUDI local de test, Podman est utilisé (voir `PLAN_ENVIRONNEMENT_RUDI_SOURCE.md`).
...
### Géocodage RVA (optionnel)

Nécessaire uniquement pour `src/monitor.py --geocode`. Créer `src/conf/rva_key.json` :

```json
{"key": "..."}
```

### Monitoring PostGIS + Superset (optionnel)

```bash
cp .env.example .env       # POSTGRES_PASSWORD, SUPERSET_SECRET_KEY, MAPBOX_API_KEY
cp src/conf/monitor_db.example.json src/conf/monitor_db.json
docker compose up -d
```
...
Menu interactif à 21 actions réparties en 4 sections : Moisson, Pipeline & publication, Maintenance, Données & infos.
...
## Monitoring & Superset (optionnel)

Sous-système entièrement facultatif — le pipeline de moisson n'en dépend jamais.

```bash
docker compose up -d          # PostGIS (127.0.0.1:5433) + Superset (127.0.0.1:8088)
python3 src/monitor.py --init-db
python3 src/monitor.py --full   # métriques + données filtrées + référentiels
```

Le provisionnement des dashboards Superset se fait depuis le conteneur (`superset/setup_full.py`) — voir la docstring du script pour la procédure complète.
...
src/
  ...
  monitor.py                                # monitoring PostGIS (optionnel)
...
docs/, PLAN_*.md, PROCEDURE_*.md            # documentation de chantiers ponctuels
